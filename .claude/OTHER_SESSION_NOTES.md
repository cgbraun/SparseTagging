...
Note: Claude Code's settings schema does not support `excludePatterns` field for excluding files from file watching. The previous attempt to add this field to prevent file watcher recursion was rejected with validation error: "Unrecognized field: excludePatterns". The schema provides no mechanism to exclude files from watching, suggesting this may be a limitation in Claude Code's architecture.
Why Notable: Identifies a potential gap in Claude Code's configuration capabilities - no way to prevent file watcher from triggering on specific files, which could cause recursion issues when tools modify files in monitored directories.
---
