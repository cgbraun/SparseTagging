...
class PerformanceBenchmark:
    ...
    def __init__(self, size_config: str = "large"):
        """
    ...
    def initialize_data(self) -> None:
        ...
        self._log(
            f"  Sparsity:   {(1 - self.fill_percent) * 100:.1f}% ({self.fill_percent * 100:.1f}% fill)"
        )
    ...
    def benchmark_single_column_queries(self) -> None:
        ...
        test_cases: list[tuple[str, dict[str, Any], int, str, Any]] = [
            ...
            (
                "Tag5 IN [HIGH,MED]",
                {"column": "Tag5", "op": "IN", "values": [HIGH, MEDIUM]},
                4,
                "IN",
                [HIGH, MEDIUM],
            ),
        ...
        for name, sparse_query, col_idx, dense_op, dense_val in test_cases:
            ...
            self._log(
                f"  Sparse (no cache): {time_uncached:7.3f}ms ± {std_uncached:.3f}ms ({count_uncached:5d} matches)"
            )

            # Sparse cached (first call - miss)
            self.bt_cached.clear_cache()
            time_miss, _, _ = self._query_sparse(sparse_query, True, 1, 0)
            ...
            self._log(
                f"  Sparse (cached):   {time_cached:7.3f}ms ± {std_cached:.3f}ms ({count_cached:5d} matches)"
            )
            ...
            if count_dense != count_uncached or count_dense != count_cached:
                self._log(
                    f"  ⚠ WARNING: Result mismatch! Dense={count_dense}, Uncached={count_uncached}, Cached={count_cached}"
                )
    ...
    def benchmark_multi_column_queries(self) -> None:
        ...
        test_cases: list[tuple[str, dict[str, Any]]] = [
            ...
            (
                "NOT (Tag2==LOW)",
                {"operator": "NOT", "conditions": [{"column": "Tag2", "op": "==", "value": LOW}]},
            ),
        ]
    ...
    def benchmark_cache_performance(self) -> None:
        ...
        if overhead < 10:
            self._log("  ✓ Overhead acceptable (<10%)")
        else:
            self._log("  ⚠ Overhead exceeds 10% threshold")
    ...
    def run_unit_tests(self) -> None:
        ...
        try:
            ...
            # Modify data
            new_data = sparse.csc_matrix(np.ones((self.n_rows, self.n_cols), dtype=np.uint8) * HIGH)
    ...
    def generate_summary(self) -> None:
        ...
        if "cache" in self.results:
            ...
            self._log(
                f"  • Cache overhead is only {cache['overhead_percent']:.1f}% for unique queries"
            )
    ...
    def run_all(self) -> Path:
        ...
        filename = reports_dir / f"performance_report_{self.size_config}_{timestamp}.txt"
